...
| 项 | 值 |
|---|---|
| 运行时间 | 2026-07-27 23:19 |
| 结果 | **53 / 53 通过** |
| 退出码 | 0 |
| 运行依赖 | 无需 World / PIE / 网络（被测对象为纯函数/纯逻辑）|

覆盖范围：画像分析器（链路②）· 规则解析器（链路⑥）· 四道护栏含分道归属（链路⑤）· 三个 Provider（链路④）· Intent JSON 解析 · Prompt 构建 · 决策日志编码契约 · 遭遇抽样 · 武器切换。
...
| PASS | `DecisionLog.Export_IsUtf8AndParsable` |
