...
**⚠️ 当前状态：M5-1 ~ M5-4 完成，M5-5（用真实对局数据校准）未做。**
链路已通、模拟器有正确性锚点守着，但**只在 12 条模拟流量上跑过**。
模拟报告自带样本量警告：不足 30 条时明确写「不能作为改动依据」。
**在真实对局数据出来之前，不许改动 `FingerprintScheme.CURRENT` 的任何参数。**

**模拟流量上的初步观察（⚠️ 不是结论，样本 12 条且非真实对局）**：
- `FLOOR` 与 `BUDGET` 被识别为**一一对应** —— 假设①在这批数据上成立
- 单独去掉任一个，指纹数与命中率都不变；**两个都去掉，指纹从 6 条并成 3 条、命中率翻倍**
- **改桶宽（34 / 50 / 100）对命中率毫无影响** ——
  若真实数据也如此，则「校准桶宽」根本不是主要矛盾，指纹结构才是

**M5-5 初步结果（2026-08-14，⚠️ 4 局 / 8 条，样本仍不足以改参数）**

用户真打了 4 局，产生 8 条真实流水（F0 是观察层不走 Provider，故一局 2 条）。
⚠️ **8 条远低于本项目自设的 30 条门槛，所以下面是发现，不是结论，
并且据此没有改动 `FingerprintScheme.CURRENT` 的任何参数。**
...
|---|---|---|
| current（8 条请求） | 6 | **0%** |
| 去掉 budget / 去掉 floor / 两个都去掉 | 6 | 0% |
| **再去掉 confidence（三个一起）** | **6** | **0%** |
| 去掉 history | 5 | 12.5% |
| **只留画像 + 原型 + 候选集** | **4** | **25%** |
...
**③ 画像本身的区分度极低。** 8 条请求去掉层号相关字段后只剩 4 种画像组合，
其中 `(build 桶 5, combat 桶 5, Vanguard)` **一个组合就占了 5 条**。
而当前方案把这 5 条拆成了 3 条不同的指纹。**这是命中率为 0 的直接原因。**

**④ 改桶宽基本无效**（34 有一点、50/100 完全无效）——
因为 build 实测全在 95~100，本来就都落在最高桶里。
**原计划要「校准桶宽 20」，数据说它根本不是主要矛盾。**
...
**但这需要更多数据来验证，而且改它会让线上已攒的缓存全部失效。**
**在样本量达到 30 条以上之前，不许动。**

**⑤ 方法论上的一个自我纠正**：最初的对照集只到「floor + budget 一起去掉」，
在我灌的模拟流量上命中率翻倍，**看着已经像结论了**。
真实数据显示它毫无效果 —— **连对照集本身都是拍的，也要由数据来纠正。**
