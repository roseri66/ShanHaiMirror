...
> **仍然有效（本项目的架构地基）**：§1 分层架构、§2.4 Tag 驱动的武器系统、§3 AI Director 系统设计、§5.2 GAS 选型结论、§7.6 接口约定。
> **已 OUT OF SCOPE**：§0 与 §7.4/§7.5 美术管线与合规（D-06/D-07）、§2.3 第二角色（D-03）、§2.6 强化系统（D-09）、§2.7 节点图与房间模板（D-10）、§6 排期与 30 分钟策略（D-01）、§8 评委视角（改称"设计自审"，内容保留）。
> **已被 DECISIONS §4.2 收紧**：`FDirectorDecision` 拆分为 `FDirectorIntent` / `FDirectorDecision` 两个类型。

# 《山海镜》技术设计文档（TDD）v0.3

> 基于 GDD v0.3 与 Design Bible v1.1 转化
> 面向：**1 程序（独立开发）× 约 7 周 MVP**（截止 2026-09-15）
> 引擎：Unreal Engine 5.5（俯视角，键鼠双摇杆瞄准 twin-stick）
> 定位：AI Native Roguelike，参加腾讯游戏开发大赛
> **Demo 目标：单局约 30 分钟可玩体验**（较 GDD 的 15 分钟翻倍，靠 AI 驱动的遭遇变化 + 模块化房间复用撑起，而非翻倍内容；单人版设 5 层 ~24min 保底，见 §6）
> LLM 通道：GameNPC（腾讯游戏开发大赛官方通道）
> 战斗系统：**自研轻量组件 + GameplayTags，不使用 GAS**（团队无 GAS 经验，已定）
> 美术策略：**AI 生成美术 + 原创技术美术**（美术成员已退出，见 §0 变更说明）
> 编写视角：Technical Director
>
> **v0.3 变更（重大）**：美术成员因时间原因退出，项目转为**单人开发**。① 团队从"1 程序 + 1 美术"改为"1 程序独立开发" ② 美术策略从"手工原创资产"改为"AI 生成美术 + 原创技术美术"（比赛规则要求原创+完整独立知识产权，第三方授权/采购资产一律不用；AI 生成美术经组委会确认允许） ③ 排期压缩为约 7 周（截止 2026-09-15） ④ 30min 保底策略（5 层 + 跨局记忆）从"备选"升为"主选" ⑤ §7 协作规范从"两人协作"改为"单人 + AI 美术管线 + 资产合规"
> **v0.2 变更**：① Demo 时长 15min→30min ② LLM 通道锁定 GameNPC ③ 战斗确定为 twin-stick 键鼠瞄准 ④ GAS 选型锁定为自研（不再保留 GAS 子集备选）

---

# 0. 变更说明（v0.3 · 美术退出 → 单人开发）

**背景**：原计划"1 程序 + 1 美术 × 8 周"。美术成员因时间原因退出，项目转为 1 程序独立开发，截止 2026-09-15（约 7 周）。当前已完成 Sprint 1 的**程序部分**（战斗切片：移动/瞄准/近战攻击/敌人 AI/接触伤害/死亡），美术部分（HP 血条、正式关卡视觉、正式模型）未做。

**核心判断：这次变更命中的是"皮"，不是"脑"。** AI Director 全闭环（观察→画像→决策→应用→解释）是 100% 代码 + LLM，比赛卖点"AI is the Gameplay"的所有技术支柱完全不依赖美术，一根汗毛没伤。受冲击的只是视觉呈现层。

**美术策略（V2）：从"手工原创资产"转为"AI 生成美术 + 原创技术美术"。**

| 资产类型 | 单人版做法 | 知识产权 |
|---|---|---|
| UI / 白泽头像 / 卡面 / 概念图 / 背景（2D） | 图像 AI（选条款授予输出所有权的工具）+ 人工编辑 | 你原创，AI 允许 |
| 敌人 / 角色 / Boss（3D） | AI-3D 生成（Meshy/Tripo 等付费档）+ 引擎内改材质；或原创风格化几何体 | 你原创 |
| 房间 / 场景 | 原创模块化几何体 kit-bash + AI 生成贴图 + 技术美术（光照/后处理） | 你原创 |
| 特效 / 材质 / 镜面视觉 | Niagara + Shader，纯技术美术 | 你原创 |

**三条法律护栏（写死）**：① 只用条款明确授予输出所有权/商用权的 AI 工具；② 绝不模仿具体版权物、不喂受版权保护的素材；③ 每个资产留创作台账（工具+日期+prompt+人工编辑步骤），作为"原创+人类创作投入"的证据。**第三方授权/采购资产（含 Epic Paragon/Quixel/商城包）一律不用**——比赛要求"完整独立知识产权"，授权 ≠ 所有权。

**美学定位**：不拼写实精度（拼不过有美术的团队），拼**风格统一**——水墨/几何风山海经 + 镜面碎裂视觉，靠后处理 + 色调 + AI 2D 堆出高辨识度的"镜界"。
...
2. **内容全部数据驱动。** 逻辑与数值分离——系统代码不写死数值，内容通过 DataTable + Data Asset + GameplayTag 扩展。**单人版价值更大**：数据驱动让你调内容/加敌人/改数值时不必回头改系统代码，AI 生成的美术资产也只需挂到既有的蓝图子类 + 数据行上，管线不重构。
...
- **几何(单人版)**:房间布局由**程序用原创模块化几何体 kit-bash 出 3-4 个房间模板**(primitive/BSP 拼装 + AI 生成贴图/材质 + 技术美术光照,全部原创),存`DT_RoomTemplate`,带标签(开阔/狭窄/多掩体)。生成器按 AI Director 的战场偏好(远程玩家→多掩体房)选模板。**模板复用是关键**:3-4 个模板 × AI 不同的刷怪/规则组合 = 几十种不重样的战斗体验,几何量不用线性增长。房间是建筑性的,不需要有机建模,是单人版最可行的场景来源。
...
**⚠️ 30 分钟的真实代价(单人版必须正视)**:压力落在 **AI 美术管线(房间模板 + 敌人/精英外观) + 技术美术(光照/后处理/材质统一) + 调平衡(层难度曲线、遭遇预算)** 上,核心系统代码几乎不增加。美术成员退出后这些全由一人扛,因此**30min 保底策略(5 层 ~24min + 跨局记忆)从"备选"升为"主选"**:主攻 5 层做扎实,用"鼓励第二局看 AI 跨局适应"补足到 30 分钟体验;6 层仅在时间充裕时作为拉伸目标(见 §6 保底策略)。
...
> 单人版:两列都由同一人做,但**时间上分两条轨**——系统轨(C++,关键路径)优先保护,内容/表现轨(蓝图 + AI 美术 + 技术美术)并行穿插。分列的意义是"哪些是纯代码、哪些要挂 AI 生成资产"。

| C++ / 系统轨(关键路径) | 蓝图 / 内容表现轨(AI 美术 + 技术美术) |
|---|---|
| EventBus、BehaviorRecorder、ProfileAnalyzer、DirectorCore、AIService(HTTP+JSON)、RuleManager | 具体敌人 BP(继承 C++ 基类,挂 AI-3D 生成/原创几何模型)、VFX(Niagara)、动画蓝图 ABP |
| 属性/能力/武器/Build 组件 | UI 控件 WBP(导演报告/HUD/三选一,挂 AI 生成 2D + 技术美术风格化) |
| ASHMCharacter / ASHMEnemy / AProjectile 基类 | 房间关卡(原创模块化几何 kit-bash)、掉落物摆放 |
| FloorGenerator / EncounterManager | 具体技能的表现层(前摇特效/音效挂点,音效用 CC0/自制) |
...
- **一切占位优先(单人版)**:先用**引擎 primitive / 简单原创几何体**占位把功能跑通,系统轨不等美术;数值先拍脑袋填 DataTable,后调。**占位一律用原创几何或 AI 生成资产,不用第三方授权/采购模型**(比赛要求完整独立知识产权)。占位到正式的替换,就是把原创几何换成 AI-3D 生成模型 + 技术美术材质。
- **每周五出一个能从头玩到当前进度的构建**(对齐 Design Bible ch21 "Playable Every Week")。

---

# 6. MVP 优先级规划(单人版 · 约 7 周,截止 2026-09-15)

> **现状**:Sprint 1 **程序部分已完成入库**(战斗切片:移动/瞄准/近战攻击/敌人 AI/接触伤害/死亡;C++ 系统 + 蓝图 + GameplayTag 注册 + 测试关卡)。tag `sprint-1-done`。美术部分(HP 血条、正式关卡视觉、正式模型)未做,转入 AI 美术/技术美术管线。
>
> **排期原则(单人版)**:AI Director 核心(卖点,纯代码)留在关键路径优先保护;视觉走"AI 生成美术 + 原创技术美术"独立轨、时间盒;砍内容广度(6→5 层保底、少 bespoke 外观)以保住 AI 卖点深度;视频/打磨提前留缓冲(程序现在兼做视频)。
>
> 下面的阶段划分是原 8 周框架的结论沿用;**逐周可执行的 7 周计划见桌面《山海镜_第二周计划.md》及后续周计划,与本节保底策略一致。**
...
> **30 分钟的负载提醒(单人版)**:内容期压力集中在 AI 美术管线(房间模板 + 敌人/Boss 外观)和调平衡。**AI 美术轨必须从第一周就并行启动、时间盒推进**——战斗切片能玩时,就该在同步跑 AI-3D 生成怪 + AI 2D 做 UI + 技术美术定"镜界"风格基线,不要等系统全好才动视觉。单人扛不动 6 层全量内容时,**主攻 5 层保底**(见下)。
...
## 30 分钟达标策略与保底(单人版:5 层为主选)

- **主路径(单人版目标)**:**5 层单局 ≈ 24 分钟 + 简版跨局记忆**——演示时打第二局,白泽用"你上局也靠弓,这次我提前布置好了"体现跨 Run 学习,两局合计轻松 30min+,而且**跨局适应本身是更强的 AI Native 卖点**。美术退出后单人扛不动 6 层全量内容,因此把 5 层 + 跨局记忆从原"保底"正式**升为主选**,"简版跨局记忆"从加分项提到**核心储备**。
- **拉伸目标(时间充裕才做)**:6 层单局 ≈ 28-32 分钟。仅当核心闭环 + 5 层已扎实、且 AI 美术管线跑顺时才拉伸,不为它牺牲卖点深度或打磨时间。
- **红线**:无论哪条路径,**单局核心闭环(观察→画像→决策→应用→解释)必须在计划中段(约第 4 周)已成立**。时长是内容问题,不能反过来拖垮核心系统。

## ❌ 看似酷但必须放弃(写死在文档里,防止跑偏)

| 放弃项 | 原因 |
|---|---|
| 运行时程序化地形生成 | 单人做不完,对核心卖点零贡献 |
...
| **第三方授权/采购资产(Paragon/Quixel/商城包)** | **比赛要求原创 + 完整独立知识产权,授权 ≠ 所有权,一律不用;美术全走 AI 生成 + 原创技术美术** |
| **bespoke 电影级 Trailer** | 无美术,改用引擎内 Gameplay 实录 + 镜界时间轴作为比赛视频,更契合 AI Native 叙事 |
| **写实级美术精度** | 单人拼不过有美术的团队,改拼风格统一(水墨/几何 + 镜面视觉),扬长避短 |

---

# 7. 单人开发规范 + AI 美术管线 + 资产合规

> v0.3:美术成员退出,本节从"两人协作"改为"单人开发"。原双人协作的核心难题(二进制资产冲突、文件锁、所有权分区)大部分消失;新增的重点是 **AI 美术管线** 和 **资产知识产权合规**。

## 7.1 双轨时间管理(替代原"所有权分区")

单人开发没有互相阻塞问题,但有**注意力争夺**问题。把工作分两条轨,防止陷在视觉里耽误卖点:

| 轨 | 内容 | 优先级 |
|---|---|---|
| **系统轨(关键路径)** | `/Source/**` 全部 C++、AI Director、战斗、遭遇、关卡流程 | **最高,永远先保证它推进** |
| **内容/美术轨(并行时间盒)** | AI 生成模型/2D、技术美术(材质/光照/后处理/Niagara)、UI、房间 kit-bash、DataTable 填数 | 时间盒,穿插做,不侵占系统轨 |

**纪律**:每周系统轨必须有可玩进度;美术轨用固定时间盒(如每天 1-2 小时或每周固定 1 天),做到"够看"即止,不追完美。**卖点(AI Director 可见性)一寸不让,视觉够风格统一即可。**

## 7.2 接口契约(单人版仍然关键)

**接口契约 = C++ 基类 + DataTable 行结构 + GameplayTag 注册表。** 单人开发时它的价值不是"不等队友",而是**让系统与内容解耦**:系统轨写好后,内容轨只需挂 AI 生成资产到既有蓝图子类 + 数据行,不回头改代码。加个敌人 = `DT_Enemy` 加一行 + 一个 BP 子类 + 一个 AI 生成模型,系统零改动。

## 7.3 Git 工作流(单人极简)

- **主干开发 + 短特性分支**,不搞复杂 GitFlow。
- **提交粒度小、频繁**;每周出一个能从头玩到当前进度的构建;每 Sprint 打 tag(`sprint-1-done` 已有)。
- **`.gitignore`(UE 必备,已配置)**:`Binaries/ Intermediate/ Saved/ DerivedDataCache/ .vs/ *.sln`
- **Git LFS**:`.uasset .umap .fbx .png .wav .mp4` 等二进制资产走 LFS(单人也建议配,AI 生成资产会有一定体积)。**当前仓库尚未配置 LFS,资产量上来前需补上 `.gitattributes`**:
  ```
...
## 7.4 AI 美术管线(单人版视觉产出方式)

| 资产 | 工具方向 | 产出 |
|---|---|---|
| UI / 白泽头像 / 卡面 / 概念 / 背景(2D) | 图像 AI(选授予输出所有权的工具)+ 人工编辑 | 导演报告/HUD/白泽的"脸" |
| 敌人 / 角色 / Boss(3D) | AI-3D 生成(Meshy/Tripo/Rodin 付费档)+ 引擎内改材质;或原创风格化几何 | 挂到 `ASHMEnemy`/`ASHMCharacter` 蓝图子类 |
| 房间 / 场景 | 原创模块化几何 kit-bash + AI 生成贴图 + 技术美术光照 | `DT_RoomTemplate` 的 Level Instance |
| 特效 / 材质 / 镜面视觉 | Niagara + Shader(纯技术美术,原创) | VFX、规则生效反馈、镜界视觉 |
| 音效 | CC0 素材库 / 自制 | 攻击/受击/UI 音 |

## 7.5 资产知识产权合规(比赛硬要求,写死)

比赛规则:参赛作品必须原创、享有**完整独立知识产权**、不得有版权纠纷。因此:

1. **第三方授权/采购资产一律不用**(Epic Paragon/Quixel/Fab 商城包等)——授权 ≠ 完整独立知识产权,踩线。
2. **AI 生成资产**(组委会已确认允许):只用**条款明确授予输出所有权/商用权**的工具;必须有实质人类创作投入(prompt 迭代 + 人工编辑),对齐中国 AI 著作权判例的认定标准。
3. **绝不模仿具体版权物**:不 prompt"某游戏/某画师风格",不喂受版权保护的素材。
4. **资产合规台账**:建 `Docs/资产台账.md`,每个资产记 工具 + 日期 + prompt + 人工编辑步骤 + 授权来源。这是被质疑时证明"原创 + 完整独立知识产权"的证据链。
5. **引擎默认内容**(Mannequin/Starter Content)属第三方,严格读也有争议——建议向组委会确认一句;在此之前占位一律用**原创几何**,不依赖引擎自带模型作为最终交付。

## 7.6 减少返工的接口约定

1. **新内容 = 加数据行,不改代码**。
2. **GameplayTag 先行**。任何新武器/敌人/规则先在注册表加 Tag,系统以 Tag 通信。
3. **DirectorDecision 是玩法层与 AI 层的唯一接口**。玩法层只认这个结构体,不关心它来自 LLM 还是兜底表。
4. **每个系统留 Debug 面板**(CheatManager):能手动灌画像、强制指定导演决策,无需真打就能测内容与调平衡。
