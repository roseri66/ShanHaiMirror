> ⚠️ **愿景存档，整体 OUT OF SCOPE。** 项目已重新定位为**求职作品集项目**（单人 · 无美术 · 4-6 周冻结），实际范围以 [`DECISIONS.md`](./DECISIONS.md) 为准。
> 本文 21 章内容**均不进入实现计划**，仅作为设计思路留档。其中 Mirror Cognitive Architecture 五层认知、AI Economy、A/B 框架、Persistent AI 等在 TDD 阶段就已明确不做。

# 《山海镜》Design Bible v1.1

> 基于 GDD v0.3 与 TDD v0.2 同步更新
> **v1.1 变更**：Demo 游玩时长从 ~15 分钟调整为 ~30 分钟，关卡结构从 5 层扩展为 6 层。

> **⚠️ 执行批注（2026-07-21，美术退出 → 单人开发）**：本 Design Bible 是**愿景全集**，正文不随执行变更而改写。因美术成员退出，MVP 执行层发生调整，以 GDD v0.4 / TDD v0.3 / Spec v1.1 为准，要点如下——① 团队 2 人 → **1 人独立开发**，周期约 7 周（截止 2026-09-15）；② 本文 ch3.5（山海异兽设计）、ch3.6（美术风格 Art Direction）、Boss/角色视觉等的 **MVP 落地方式改为「AI 生成美术 + 原创技术美术」**（比赛要求原创+完整独立知识产权，第三方授权/采购资产不用；AI 生成美术经组委会确认允许），美学定位从写实精度转向**风格统一（水墨/几何 + 镜面视觉）**；③ 单局以 5 层 ~24min + 跨局记忆为主选、6 层为拉伸。**核心愿景（AI 导演闭环、透明性、玩家画像）是纯代码，完全不受美术退出影响，本文相关章节全部有效。**
...
| 开发周期 | 约 7 周（截止 2026-09-15）              |
| 团队规模 | 1 人（独立开发，美术已退出）              |
| 目标赛事 | 腾讯游戏开发大赛                           |
| 当前版本 | GDD v0.4 / TDD v0.3 / Spec v1.1 / Design Bible v1.1 |
...
* 一局约 30 分钟的流程能够完整体现"观察—分析—调整—适应"的 AI 对抗闭环。
