...
> ② **聚合能力已具备**（第八次开工，落库 + 模拟器，见下）
> ——**但用它校准缓存参数这一步还没做**，因为需要真实对局数据，
> 目前只在模拟流量上验证过链路。
...
而**画像本身几乎没有区分度**：8 条请求里，
`(build 桶 5, combat 桶 5, Vanguard)` 一个组合就占了 5 条，
却被拆成了 3 条不同的指纹。**这是命中率为 0 的直接原因。**

原计划要校准的**桶宽根本不是主要矛盾**——改到 50、100 都毫无影响
（build 实测全在 95~100，本来就都在最高桶里）。

> ⚠️ **但 8 条远低于我自设的 30 条门槛，所以这是发现不是结论，
> 据此没有改动任何参数。** 报告会自己说这句话。
> 拿十几条数据说「我校准了参数」，正是这个项目在演示数据那次犯过的错。
>
> ⭐ 顺带一个自我纠正：最初的对照方案集只到「floor + budget 一起去掉」，
> 在我灌的模拟流量上命中率翻倍、**看着已经像结论了**，真实数据一跑毫无效果——
> **连对照集本身都是拍的，也要由数据来纠正。**
...
mvn test                     # 79 个测试（其中 9 条需 Docker，无 Docker 会跳过）
```
...
| **决策数据落库与聚合**（`DirectorService/`，D-24） | ✅ 链路已通 · 原始字段拆列存储 · 指纹方案模拟器（**含正确性锚点：重放必须逐条重现真实历史**）· 三条硬约束有测试守着 · ⚠️ **参数校准待真实对局数据**，当前仅在模拟流量上验证 |

开发过程记录：[`Docs/Sprint开发总结.md`](Docs/Sprint开发总结.md)（八次开工复盘，含设计判断、计划偏离与修复教训）· [`Docs/踩坑记录.md`](Docs/踩坑记录.md)（**32 条**，每条含现象/原因/解法/规则）
...
**范围与每一条取舍的理由见 [`Docs/DECISIONS.md`](Docs/DECISIONS.md)**，包含 **24 条**决策记录（砍掉什么、为什么砍、代价是什么、以及时间不够时的削减顺序）。MVP 冻结后的每一条扩展都要在 §7 先追一条显式决策——**不追决策就动手，等于自己破自己的规矩**。
