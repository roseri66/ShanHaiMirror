...
| `BehaviorRecorder`（链路 ①） | ✅ 已完成 · 尚无测试 |
| `ProfileAnalyzer`（链路 ②，五维） | ✅ 已完成 · **16 个单测全绿** |
| 单元测试（画像） | ✅ [`Docs/TestResults.md`](Docs/TestResults.md) |
| 属性 / 能力 / 武器 / Build 组件 | 🟡 骨架已有 |
| 武器切换 + 弓（画像分化的输入源） | ⬜ 未开始 |
| `DirectorCore` + 四道护栏（链路 ③⑤⑥） | ⬜ 未开始 |
| `IAIProvider` 三实现（链路 ④） | ⬜ 未开始 |
| 决策日志 + 导演报告 UI（链路 ⑦） | ⬜ 未开始 |
| 单元测试（护栏） | ⬜ 未开始 |
...
| 看什么 | 在哪 |
|---|---|
| **数据契约**（最能说明设计意图） | `Source/ShanHaiMirror/Framework/SHMCoreTypes.h` |
